#!/bin/bash
# CPT-PILOODI TÄISAHEL (nõukoja retsept, 2026-08-28):
#   korpus → baasi perplexity → CPT-treening → CPT perplexity →
#   SFT-kombo → SFT CPT peale → eval + gguf + fraasisond
# Kõik logid adapters/ all, tulemused CPT-PILOOT.md jaoks.
set -u
...
echo "=== 1/7 korpus $(date +%H:%M)"
.dl-venv/bin/python "$S/42_build_cpt_korpus.py" > "$A/cpt-korpus.log" 2>&1 || { echo KORPUS EBAÕNNESTUS; exit 1; }
tail -2 "$A/cpt-korpus.log"

echo "=== 2/7 baasi perplexity $(date +%H:%M)"
.train-venv/bin/python "$S/44_perplexity.py" --valjund "$A/ppl-baas.json" > "$A/ppl-baas.log" 2>&1 || echo "ppl-baas viga (jätkan)"
grep PERPLEXITY "$A/ppl-baas.log" || true

echo "=== 3/7 CPT-treening $(date +%H:%M)"
.train-venv/bin/python "$S/43_train_cpt.py" --run cpt1 > "$A/cpt1.log" 2>&1
...
.train-venv/bin/python "$S/44_perplexity.py" --adapter "$A/cpt1/final" --valjund "$A/ppl-cpt1.json" > "$A/ppl-cpt1.log" 2>&1 || echo "ppl-cpt viga (jätkan)"
